"""
Engulfing Pattern Backtester (MetaTrader5) – Optimized Strategy
---------------------------------------------------------------
Advanced Features:
    - Multi-EMA trend alignment (fast + slow EMA)
    - RSI confirmation (avoid overbought/oversold entries)
    - ATR-based engulfing size filter (skip tiny candles)
...
ATR_MIN_BODY_RATIO = 0.3      # Engulfing body must be >= 30% of ATR
RISK_PERCENT = 1.0
...
# ---------------------------------------------------------------------------
# Engulfing classification with quality filter
# ---------------------------------------------------------------------------
def classify_engulfing(prev_candle, curr_candle, atr=None):
    ...
    # Quality filter: engulfing body must be significant
    curr_body = abs(curr_close - curr_open)
    if atr is not None and atr > 0:
        if curr_body < atr * ATR_MIN_BODY_RATIO:
            return None  # Skip tiny engulfing candles
    ...
    for i in track(range(1, len(candles)), description="Processing candles..."):
        ...
        if USE_ADVANCED:
            ...
            if (i - last_trade_candle) < TRADE_COOLDOWN:
                ...
                pattern = classify_engulfing(prev, curr, atr)

                if pattern:
                    # Trend filter: EMA alignment
                    trend = get_trend(candles[:i+1], EMA_FAST, EMA_SLOW)
                    if trend == "bullish" and pattern == "bearish":
                        pattern = None
                    elif trend == "bearish" and pattern == "bullish":
                        pattern = None
                ...
                if pattern:
                    ...
                    if rsi is not None:
                        if rsi > RSI_OVERBOUGHT and pattern == "bullish":
                            pattern = None  # Don't buy when overbought
                        elif rsi < RSI_OVERSOLD and pattern == "bearish":
                ...
                if pattern:
                    ...
                    if sl_distance > 0:
                        # XAUUSD contract size varies by broker, use a safe default
...
def print_summary(trades, symbol, timeframe, lot, initial_balance, final_balance,
    ...
    table = Table(title=f"Backtest Summary – {symbol} {timeframe}{status_str}", box=ROUNDED)
...
<body>
<div class="container">
    <h1>Backtest Report – {symbol} {timeframe} {status_badge}</h1>
    <div class="summary">
    ...
    <h2>Trade List</h2>
    <table>
        <thead><tr><th>ID</th><th>Direction</th><th>Entry</th><th>Close</th><th>Result</th><th>Lot</th><th>Profit</th></tr></thead>
        <tbody>{trade_rows}</tbody>
    </table>
    <div class="footer">Generated by Engulfing Backtester (Optimized)</div>
</div>
</body>
</html>"""
