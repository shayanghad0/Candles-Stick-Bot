"""
Morning / Evening Star Backtester (MetaTrader5) – Optimized Strategy
----------------------------------------------------------------------
Advanced Features:
...
def run_backtest(symbol, tf_const, tf_key, lot, candle_count, initial_balance):
    ...
    console.print(f"Starting {strategy_label} Morning/Evening Star backtest on {symbol} {tf_key} with {len(candles)} candles...")
    ...
    for i in track(range(2, len(candles)), description="Processing candles..."):
        ...
        # --- 2. Detect pattern and check filters ---
        if USE_ADVANCED:
            # Cooldown check
            if (i - last_trade_candle) < TRADE_COOLDOWN:
                pass  # Skip signal check during cooldown
            else:
                # Detect patterns on the just-closed candle using 3-candle window
                ...
                if pattern:
                    # ATR filter: skip tiny patterns
                    atr = calc_atr(candles[:i+1], ATR_PERIOD)
                    if atr is not None and atr > 0:
                        # Use the third candle's body as the signal body
                        signal_body = abs(curr["close"] - curr["open"])
                        if signal_body < atr * ATR_MIN_BODY_RATIO:
                            pattern = None  # Skip tiny signal candle
...
def print_summary(trades, symbol, timeframe, lot, initial_balance, final_balance,
    ...
    table = Table(title=f"Morning/Evening Star Backtest Summary – {symbol} {timeframe}{status_str}", box=ROUNDED)
...
<body>
<div class="container">
    <h1>Morning/Evening Star Backtest Report – {symbol} {timeframe} {status_badge}</h1>
    <div class="summary">
    ...
    <div class="footer">Generated by Morning/Evening Star Backtester (Optimized)</div>
</div>
</body>
</html>"""
