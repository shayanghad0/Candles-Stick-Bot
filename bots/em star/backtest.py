"""
Morning / Evening Star Backtester (MetaTrader5) – with Balance & HTML Export
----------------------------------------------------------------------------
- Loads historical OHLC data from MT5.
- Detects Morning Star (bullish) and Evening Star (bearish) on each closed candle.
- Simulates trades with fixed TP/SL (BUY on Morning Star, SELL on Evening Star).
...
TRADES_JSON = os.path.join(BASE_DIR, "morning_star_trades.json")
SUMMARY_CSV = os.path.join(BASE_DIR, "morning_star_summary.csv")
HTML_REPORT = os.path.join(BASE_DIR, "morning_star_report.html")
...
class SimulatedTrade:
    def __init__(self, trade_id, symbol, direction, pattern, entry_price, entry_time, lot, point, tp_points, sl_points):
        ...
        self.pattern = pattern      # "Morning Star" or "Evening Star"
...
def run_backtest(symbol, tf_const, tf_key, lot, candle_count, initial_balance):
    ...
    for i in track(range(2, len(candles)), description="Processing candles..."):
        # We need the last 3 candles for pattern detection; 'i' is the index of the current candle (the one that just closed)
        # So the three candles are: i-2, i-1, i
        ...
        # --- 2. Detect patterns on the just-closed candle (prev) using the window of 3 candles ---
        # Create a small DataFrame from the last 3 candles (prev_prev, prev, curr)
        # But the pattern detection expects a DataFrame; we'll build it from the three candles.
        # We need the exact same logic as in the live bot, so we'll construct a DataFrame.
        df_slice = pd.DataFrame([
            {"open": prev_prev["open"], "high": prev_prev["high"], "low": prev_prev["low"], "close": prev_prev["close"]},
        ...
        pattern = None
        direction = None
        if detect_morning_star(df_slice):
            pattern = "Morning Star"
            direction = "BUY"
        elif detect_evening_star(df_slice):
            pattern = "Evening Star"
            direction = "SELL"

        if pattern and direction:
            # Entry at the close of the signal candle (which is the middle candle, i-1)
...
def print_summary(trades, symbol, timeframe, lot, initial_balance, final_balance):
    ...
    table = Table(title=f"Morning/Evening Star Backtest Summary – {symbol} {timeframe}", box=ROUNDED)
...
def generate_html_report(trades, equity_curve, balance_curve, symbol, timeframe, lot, initial_balance, final_balance):
    ...
    if equity_curve:
        ...
        ax1.set_title(f"Morning/Evening Star Equity Curve – {symbol} {timeframe}")
...
<head>
    <meta charset="UTF-8">
    <title>Morning/Evening Star Backtest Report – {symbol} {timeframe}</title>
    <style>
...
<body>
<div class="container">
    <h1>Morning/Evening Star Backtest Report – {symbol} {timeframe}</h1>
    <div class="summary">
    ...
    <div class="footer">Generated by Morning/Evening Star Backtester</div>
</div>
</body>
</html>"""
